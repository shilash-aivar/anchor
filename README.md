...
```bash
cd ctxly
make install-all          # ~/.local/bin/ctxly + completions
...
ctxly project add
ctxly login --all
ctxly use                   # fzf picker, or reads .ctx.yaml in repo
ctxly status
```
...
hooks:
  pre_use: ~/scripts/ctxly-pre-use.sh
  post_use: ""
  pre_apply: ""
```
...
Disable fzf: `export CTXLY_NO_FZF=1`
